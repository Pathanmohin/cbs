// ignore_for_file: non_constant_identifier_names, prefer_final_fields, no_leading_underscores_for_local_identifiers, use_build_context_synchronously, unused_local_variable, unrelated_type_equality_checks
...
class ADDADDRESSRD extends StatefulWidget {
  const ADDADDRESSRD({super.key});

  @override
  State<ADDADDRESSRD> createState() => _ADDADDRESSRDState();
}

class _ADDADDRESSRDState extends State<ADDADDRESSRD> {
          ...
          if (response.body != "") {
            ...
            if (res["Result"].toString().toLowerCase() == "success") {
              ...
              for (int i = 0; i < data.length; i++) {
                ...
                vObject.cityName = "${getState["dist_ename"]} (${getState["dist_pincode"]})";
                ListGet.add(vObject);
              }
...
//------------------------------------------------------------------------------------------------------
      //String apiUrl = "rest/AccountService/CreateRD";
...
//------------------------------------------------------------------------------------------------------
      String encrypted =
          AESencryption.encryptString(json, ibUsrKid);
      ...
      try {
        if (response.statusCode == 200) {
          if (response.body != "") {
            ...
            String deencrypted =
                AESencryption.decryptString(data, ibUsrKid);
